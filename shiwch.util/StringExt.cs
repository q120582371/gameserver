...
namespace shiwch.util
{
    ...
    {
        /// <summary>
        /// 将16进制表示的字符串转化为对应的字节数组
        /// </summary>
        /// <param name="value">要进行转换的16进制表示的字符串</param>
        /// <exception cref="ArgumentNullException">value为空</exception>
        /// <exception cref="ArgumentException">value不是16进制表示的字符串</exception>
        /// <returns></returns>
        public static byte[] ParseHex(this string value)
        {
            Check.NotNull(value, "value");
            if (value.Length % 2 != 0) throw new ArgumentException("value不是合法的16进制字符串");
        ...
        /// 将以","分割的字符串切分
        /// </summary>
        /// <param name="idList">以逗号分隔的字符串</param>
        /// <returns>切分后的数值列表</returns>
        public static List<long> ToIdListLong(this string idList)
        {
            return idList.Split(SplitChar, StringSplitOptions.RemoveEmptyEntries).Select(p => Convert.ToInt64(p)).ToList();
        }
        ...
        /// <param name="idList">以逗号分隔的字符串</param>
        /// <returns>切分后的数值列表</returns>
        public static List<int> ToIdListInt(this string idList)
        {
            return idList.Split(SplitChar, StringSplitOptions.RemoveEmptyEntries).Select(p => Convert.ToInt32(p)).ToList();
        }

        /// <summary>
        /// 判断字符串是否是数字或者26个英文字母大小写组成
        /// </summary>
        /// <param name="value">要检测的字符串</param>
        /// <returns></returns>
        public static bool IsAlphaOrNum(this string value)
        {
        ...
        /// <summary>
        /// 判断字符串是否是数字或者26个英文字母大小写或者下划线组成
        /// </summary>
        /// <param name="value">要检测的字符串</param>
        /// <returns></returns>
        public static bool IsAlphaOrNumOrUnderline(this string value)
        {
            if (string.IsNullOrEmpty(value)) return false;
        ...
        /// <summary>
        /// 字符串格式化，可以使用 {ParamName} 替换 string.Format的位置格式化参数 {0}
        /// Util.Format("You are {age} years old and your last name is {name} ", new Dictionary&lt;string, object&gt;() {{"age" = 18, "name" = "Foo"}});
        /// </summary>
        /// <returns></returns>
        public static string FormatX(string pattern, IDictionary<string, object> args)
        {
        ...
        /// <summary>
        /// 过滤字符串中的不可打印字符0x00~0x1F 0x7F
        /// </summary>
        /// <param name="value">需要过滤的字符串</param>
        /// <returns></returns>
        public static string FilterString(this string value)
        {
        ...
        {
            if (string.IsNullOrEmpty(value)) return false;
            if (Regex.IsMatch(value, @"^\d{6,8}$")) return true;
            if (Regex.IsMatch(value, @"^\d{9,12}$")) return true;
            var idx = value.IndexOf('-');
            if (idx != -1)
            {
                var t1 = value.Substring(0, idx);
                var t2 = value.Substring(idx + 1);
                if (Regex.IsMatch(t1, @"^\d{3,4}") && Regex.IsMatch(t2, @"^\d{6,8}")) return true;
                return false;
            }
            ...
            {
                var t1 = value.Substring(idx1 + 1, idx2 - idx1 - 1);
                var t2 = value.Substring(idx2 + 1);
                if (Regex.IsMatch(t1, @"^\d{3,4}") && Regex.IsMatch(t2, @"^\d{6,8}")) return true;
                return false;
            }
            return false;
        ...
        private static string[] mobilePrefix = new string[]{
            "134","135","136","137","138","139","147","150","151","152","157","158","159","182","183","184","187","188",//中国移动
            "130","131","132","155","156","185","186",//中国联通
            "133","134","153","180","181","189"//中国电信
        };
        /// <summary>
        /// 判断是否是手机号
        /// </summary>
        ...
        {
            string v = value;
            if (v.StartsWith("+86")) v = v.Substring(3);
            else if (v.StartsWith("86")) v = v.Substring(2);

            if (v.Length != 11) return false;
            else if (!v.IsNumber()) return false;
            else
            {
                foreach (var p in mobilePrefix)
                {
        ...
        {
            if (string.IsNullOrEmpty(value)) return false;
            return Regex.IsMatch(value, @"^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$");
        }
        ...
        /// 判断字符串是否符合身份证格式
        /// </summary>
        /// <param name="cid">要判断的字符串</param>
        /// <returns></returns>
        public static bool IsIDCardNum(this string cid)
        {
            if (string.IsNullOrEmpty(cid)) return false;
            if (cid.Length == 15)
            {
                if (!Regex.IsMatch(cid, @"^\d{15}")) return false;
                DateTime tmp;
                if (!DateTime.TryParse(cid.Substring(6, 2) + "-" + cid.Substring(8, 2) + "-" + cid.Substring(10, 2), out tmp)) return false;
                return true;
            }
            else if (cid.Length == 18)
            {
                if (!Regex.IsMatch(cid, @"^\d{17}(\d|x|X)$")) return false;
                DateTime tmp;
                if (!DateTime.TryParse(cid.Substring(6, 4) + "-" + cid.Substring(10, 2) + "-" + cid.Substring(12, 2), out tmp)) return false;
                double iSum = 0;
                cid = cid.ToLower().Replace("x", "a");
                for (int i = 17; i >= 0; i--)
                {
